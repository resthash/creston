import { ref, push, onChildAdded, serverTimestamp, auth, db, onValue, update, set } from "./firebase.js";
...
function listenForAdminTopups(uid) {
    const activityRef = ref(db, `users/${uid}/activity`);
...
// Initialize the listener after auth loads
auth.onAuthStateChanged(user => {
    if (user) listenForAdminTopups(user.uid);
});
...
async function notifyUser(type, userData, extraData = {}) {
    const templates = {
        kyc: "template_kyc_id",
        balance: "template_balance_id",
