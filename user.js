import { ref, push, onChildAdded, serverTimestamp, auth, db, onValue, update, set } from "./firebase.js";import { onAuthStateChanged } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-auth.js";

let userData = null;let prices = {};let currentActiveCoin = "";let currentCoinPrice = 0;let currentCoinBalance = 0;

window.openConnectModal = () => {const modal = document.getElementById('connectWalletModal');if (modal) {modal.style.display = 'flex'; // This activates the centering logic in the CSS}};
...
onValue(userRef, async (snapshot) => {
    ...
    if (user) {
    // ... your other logic ...
    setupNotificationListener(user.uid, db);
}  
});
...
function switchView(view) {const views = {'home': document.getElementById('view-home'),'settings': document.getElementById('view-settings'),'explore': document.getElementById('view-explore')};
...
function openTxnForm(coin, balance) {currentActiveCoin = coin;currentCoinBalance = balance;currentCoinPrice = prices[coin]?.price || 0;
...
function renderActivity(activityData) {const container = document.getElementById("activity-container");if (!container) return;
...
onChildAdded(activityRef, async (snapshot) => {
    ...
    // If the admin marked this as a new credit
    if (data && data.isNew === true && data.type === "credit") {
        
        // 1. Show the Pop-up (using a simple alert or your custom UI)
        showTopupNotification(data.amount, data.asset, data.address);

        // 2. Mark as 'seen' so it doesn't pop up again
        const itemRef = ref(db, `users/${uid}/activity/${snapshot.key}`);
        await update(itemRef, { isNew: false });
    }
});
